...
function _decodePacked(body) {
    ...
    try {
        ...
        log(`[SnapSave] decode fail: ${e.message}`, 'WARN');
    }
...
function _extractStringLiteral(source, search) {
    ...
    let i = pos + 1, escaped = false, literal = quote;
    ...
    try { return vm.runInContext(`tmp=${literal}`, vm.createContext({})); }
    catch {
...
async function downloadAll(link) {
    const url = (link || '').trim();
    ...
    try {
        ...
        if (fallback) {
            return {
                title: 'Facebook Video', author: 'SnapSave', thumbnail: null,
                medias: [{ type: 'video', quality: 'HD', url: fallback,
                    extension: fallback.match(/\.([a-zA-Z0-9]{2,4})(?:[?&#]|$)/)?.[1] || 'mp4' }],
            };
        }
        return { error: true, message: 'Không tìm thấy link download' };
    } catch (err) {
        log(`[SnapSave] ${err.message}`, 'WARN');
        return { error: true, message: 'Lỗi tải media SnapSave' };
    }
}
...
module.exports = {
    downloadAll,
    pickBestVideo,
    name: '/download/snapsave',
    index: async (req, res) => {
        const url = req.query.url;
        if (!url) return res.status(400).json({ status: false, message: "Thiếu tham số 'url'", example: '/download/snapsave?url=https://www.facebook.com/...' });
