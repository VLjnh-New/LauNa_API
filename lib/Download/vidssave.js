...
async function queryDownloadLink(taskId, { timeout = 60000 } = {}) {
...
async function getDownloadLink(resourceContent, options = {}) {
...
async function downloadAll(link, { resolveLinks = true, resolveLimit = Infinity, timeout = 60000 } = {}) {
    try {
        ...
        if (resolveLinks) {
            ...
            for (const media of targets) {
                try {
                    ...
                    log(`[VIDSSAVE] Không lấy được link cho ${media.type} ${media.quality || ''}: ${e.message}`, 'WARN');
                }
            }
        }
        return data;
    } catch (e) {
        log(`[VIDSSAVE] downloadAll lỗi: ${e?.message || String(e)}`, 'ERROR');
        return { error: true, message: e?.message || String(e) };
    }
}

module.exports = {
    name: '/download/vidssave',
    index: async (req, res) => {
        const url = req.query.url;
        const resolveLinks = String(req.query.resolve ?? 'true').toLowerCase() !== 'false';
        const resolveLimit = req.query.limit ? Math.min(50, Math.max(1, Number(req.query.limit) || 1)) : 20;
        const timeout = req.query.timeout ? Math.min(120_000, Math.max(5000, Number(req.query.timeout) || 60000)) : 60000;

        if (!url) {
            return res.status(400).json({
                status: false,
                message: "Thiếu tham số 'url'",
                example: '/download/vidssave?url=https://www.tiktok.com/...'
            });
        }

        const data = await downloadAll(url, { resolveLinks, resolveLimit, timeout });
        if (data.error) return res.status(500).json({ status: false, message: data.message });
